...
class Driver_auth extends CI_Controller {
  ...
  public function register(){
    ...
    if($this->input->post('do_register')){
      ...
      if($name=='' || $email=='' || $pass==''){
        ...
        $this->session->set_userdata('driver_id', $id);
        $this->session->set_flashdata('ok','Account created. Please upload documents for verification.');
  ...
  public function login(){
    ...
    if($this->input->post('do_login')){
      ...
      $pass = trim($this->input->post('password', true));
      if($email=='' || $pass==''){
        $data['err']='Enter email and password.';
      } else {
        $u = $this->auth->login('driver',$email,$pass);
        if($u){
          $this->session->set_userdata('driver_id',$u['user_id']);
          redirect('driver/driver_dashboard');
  ...
  public function logout(){
    $this->session->unset_userdata('driver_id');
